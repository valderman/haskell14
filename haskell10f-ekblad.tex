...
\exclusivelicense
\conferenceinfo{ICFP~'14}{September 6, 2014, Gothenburg, Sweden}
\copyrightyear{2014}
...
window.onload = function() {
  ...
  sock.onmessage = function(e) {
    log.value = e.data + NEWLINE + log.value;
  };
...
design. Two UI elements, \lstinline!log! and \lstinline!msg!, represent
...
The part of the program contained within the \lstinline!App! monad is executed
on both the server and the client, albeit with slightly different side effects,
as described in section \ref{sec:impl}.
...
\begin{code}
setup :: IO ()
setup = runApp cfg main
  where cfg = mkConfig "ws://localhost:1111" 1111
\end{code}
...
An \lstinline!MVar! is associated with the
...
function's argument, wrapped in the \lstinline!Server! monad in order to
prevent client-side inspection.
...
the \lstinline!Chan! corresponding to each active session.
...
appropriate \lstinline!Chan! to wait on by searching the recipient list for the
...
an exception will be thrown to the client. No server side state needs to be
cleaned up as the very lack of such state was our reason for dropping the
session.
...
with the identifier ``log'', and \lstinline!msg! will be bound to the node
identified by ``msg''. These are the same DOM nodes that were referenced in
...
\begin{code}
      let recvLoop hist = do
            setProp log "value" $ unlines hist
            message <- onServer awaitMsg
            recvLoop (message : hist)
      fork $ recvLoop []
\end{code}
...
\begin{code}
      msg `onEvent` OnKeyPress $ \13 -> do
        msgtext <- getProp msg "value"
        setProp msg "value" ""
        onServer (sendMsg <.> msgtext)
\end{code}
...
\begin{code}
newtype Server a =
  Server (ReaderT SessionInfo IO a)
...
onEvent mapping json = do
  let (nonce, ident, args) = fromJSON json
      Just f = lookup ident mapping
  result <- f args
...
This concurrency-enhanced IO monad is used as the basis of the
\lstinline!Client! monad.
...
The \lstinline!createResultMVar! function creates a new \lstinline!MVar!,
paired with its corresponding nonce in the 
...
-- A MutableVar is completely opaque to Haskell
-- code and is only ever manipulated in
-- JavaScript. Thus, we use the Unpacked type
-- to represent it, indicating a completely
-- opaque value.
