...
is contained within the \lstinline!Server! monad, completely out of reach of the
client. Any client side code is thus free to completely ignore executing
...
type, the server has no way to inspect \lstinline!Remote! functions, and thus
only has a no-op implementation of the \lstinline!<.>! operator. On the other
hand, it does have full access to the values and side effects produced by
the \lstinline!liftServerIO! function, which is quite simple. The side effect
is performed immediately and its result saved for later, as state holding
elements should only be initialized once.
