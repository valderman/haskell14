...
and add the \lstinline!setup! function in figure \ref{lst:setup} to the source
code. This will instruct the server binary to listen on the port 1111 when
...
\end{code}
\caption{Bootstrapping a Haste.App program}
\label{lst:setup}
\end{listingfloat}
...
\begin{code}
data RemoteValue a = Here a | There
...
\lstinline!Remote! constructor. The counterpart function, \lstinline!use!
simply unwraps the \lstinline!Here! constructor and returns the contained
value in the \lstinline!Server! monad.

\begin{code}
use :: Remote a -> Server a
use (Here x) = return x
\end{code}

\lstinline!remote! is a more interesting function. Looking at its type, we see
that it accepts any \lstinline!Remotable! value for its argument.
...
The client implementation of \lstinline!remote! is not very complex.
...
The second argument of the \lstinline!Remote! constructor is the list of
arguments to be sent to the function, to be updated as the remote function is
