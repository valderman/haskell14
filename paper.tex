...
\paragraph{Opa} The Opa framework\ \cite{opa}, much like Conductance, enables
seamless communication within the JavaScript language itself.
While Opa improves upon Conductance by introducing non-mandatory type checking
to the JavaScript world, its communication model is based on implicit
information flows, allowing the server to read and update mutable state on the
client and vice versa. While this is a quite flexible programming model, we
believe that this uncontrolled, implicit information flow makes programs harder
to follow, debug, secure and optimize.
...
within existing languages, there are several languages specifically created to
target the web domain. These languages tend to target not only the client and
server tiers, but the database tier as well. These languages bring many new,
interesting ideas to the table but suffers from not being compatible with any
