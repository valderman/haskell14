...
\begin{itemize}
  ...
  \item The implementation is built as a library on top of the GHC and Haste
        Haskell compilers, requiring little to no compiler support.
\end{itemize}

\paragraph{A first example} While explaining of the properties of our solution
is all well and good, nothing compares to a good old Hello World example to
convey the idea. We begin by implementing a function which prints a greeting
...
This is basically an IO monad with a few extra operations for session handling;
its main purpose is to prevent the programmer from accidentally attempting to
perform client-exclusive operations, such as popping up a browser dialog box,
on the server.
...
In order to make an RPC call using this handle, we must supply it with an
argument. This is done using the \lstinline!<.>! operator. Upon closer
inspection, the attentive reader may notice that its type,
\linebreak
...
serialized and sent over a network connection.
