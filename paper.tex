...
Javascript \texttt{Number} type, which is defined as the IEEE754 double
...
values representing its arguments. For instance, the value \texttt{42 :: Int}
is represented as \texttt{[0, 42]}, the leading \texttt{0} representing the
zeroth constructor of the \texttt{Int} type and the \texttt{42}, obviously,
representing the ``machine'' integer. It may seem strange that a limited
precision integer is represented using one level of indirection rather than as
a simple number, but recall that the \texttt{Int} type is defined by GHC as
\texttt{data Int = I\# Int\#}.
...
and made explicit by the use of the \texttt{onServer} function, with the
...
\texttt{onClient} counterpart to \texttt{onServer} would be relatively
straightforward.

While our solution completely safe from the invocation point of the
\texttt{runApp} function onward, capturing IO-dependent variables introduced
...
The reason for this is that the \texttt{randomRIO} computation will be executed
twice - once on the client, and once on the server. Whenever the computation
returns differening results, the client and the server will have conflicting
opinions on the actual value of \texttt{captured}, causing any code
which - quite reasonably - assumes that variables are consistent between
client and server to fail.

This is easily remedied however, by a slight compiler modification. Instead of
using \texttt{main :: IO ()} as the program entry point, introduce a function
\texttt{appMain :: App Done} for this purpose, and let the runtime system start
the application by invoking \texttt{runApp config appMain}, where
\texttt{config} contains the URL and port configuration for the server, given
...
\texttt{unsafePerformIO} to declare IO-dependent top-level values, and that
...
any Haskell program using \texttt{unsafePerformIO}, even subverting the type
