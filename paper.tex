...
fast learner, and so manages to produce the following client-side code in
figure \ref{lst:javascript-client}.
...
enhanced with new features later, but the client is not willing to pay Jen to
...
will it take to ensure that the two code bases stay in sync? How much code
will need to be written twice, one version for the client and one for the
server, due to the unfortunate fact that the two can't share any code?

Coming from a desktop background, Jen wonders why something so seemingly simple
should involve client/server architectures and network protocols at all.
