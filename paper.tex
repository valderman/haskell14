...
\titlebanner{-- ICFP '14 draft submission --}        % These are ignored unless
\preprintfooter{-- -- -- -- draft -- -- -- --}   % 'preprint' option specified.

\title{Type-safe client-server communication in functional web applications}
...
being wonky and error-prone, and the other in the current server language
\emph{du jour}.
...
As our implementation depends on the Haste Haskell to Javascript compiler, this
paper also goes into some detail about its design and implementation.
...
named \lstinline!Serialize! type class, and produces a typed identifier which
...
newtype App a = (StateT AppState IO a)
...
As we can see, the implementation of our programming model is very
straightforward and requires no bothersome compiler modifications or language
...
\begin{itemize}
  ...
  \item We also want our compiler to be practical, in the sense that its output
...
classes, and thus has no one to one server-side counterpart compiler.
...
spite of providing very good compatibility with even low-level GHC features.
Similarly, the code produced by the UHC Javascript backend tends to
...
supported in Haskell thanks to do-notation and a culture of monadic design
patterns. Add the fact that one of this work's authors is the original author
of Haste, giving us a better position to troubleshoot any code based thereupon,
and Haste becomes the better choice.
...
greater flexibility, but also eliminates the need to tangle with complex
compiler internals.
...
function \lstinline!appMain :: App Done! for this purpose, and let the runtime
system start the application by invoking \lstinline!runApp config appMain!,
