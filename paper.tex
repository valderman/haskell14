...
clutter in the interest of clarity.
...
monad. Its definition is as follows.
...
onServer (Export identifier args) = do
  ...
  webSocketSend $
    serialize (nonce, identifier, reverse args)  
  takeMVar mv
\end{code}
...
call's nonce and sent back to the client. Note that during normal operation,
it is not possible for the client to submit an RPC call with a non-existent
call identifier, hence the irrefutable pattern match on \lstinline!Just f!.
Should this pattern match fail, this is a sure sign of malicious tampering;
the resulting exception is caught and the session is dropped as it is no longer
