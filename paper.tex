...
is wrapped in the abstract \lstinline!RemoteValue! type. As the name implies,
...
server: the only function capable of turning a \lstinline!RemoteValue! into
one that can be inspected lives in the server monad. Any client side code is
thus free to completely ignore executing computations lifted using
\lstinline!liftServerIO!; since the result of a server lifted computation is
never observable on the client, the client has no obligation to even produce
such a value. Figure \ref{lst:good-state} shows how to make proper use of
server side state.
...
\lstinline!use! & \parbox[t]{5cm}{Extract the contents of a
\lstinline!RemoteValue! produced by \lstinline!liftServerIO! on the server.} \\
...
The workings of the \lstinline!App! monad basically hinges on two abstract data
types: \lstinline!RemoteValue! and \lstinline!Remote!. \lstinline!RemoteValue!
is used to represent values which are available to the server but not to the
client. \lstinline!Remote! denotes functions which live on the server but can
...
We begin by looking at the client side implementation.
...
As values of the \lstinline!RemoteValue! type are not accessible to the client,
they are represented by a simple dummy value. The \lstinline!Remote! type is
slightly more useful, containing the ID of a remote function and a list of the
serialized arguments to be passed when invoking it. In essence, the
\lstinline!Remote! type is an explicit representation of a remote closure.
Such closures can be applied to values using the \lstinline!<.>! operator.
...
far and thus which ID to assign to the next remote function.
...
\lstinline!RemoteValue!s are represented by a dummy value on the client, we
just return said value.

\paragraph{Server side implementations}
The server side representation of the \lstinline!RemoteValue! and
...
type but not with \lstinline!RemoteValue! the server has no way to inspect
\lstinline!Remote! functions, and thus only has a no-op implementation of the
\lstinline!<.>! operator. On the other hand, the server has full access to the
contents of \lstinline!RemoteValue!s via the \lstinline!use! function.
...
Those values are produced by the \lstinline!remoteIO! function, which is quite
simple. It merely executes its argument and wraps its argument in the
\lstinline!RemoteValue! type.

\begin{code}
remoteIO :: IO a -> Server (RemoteValue a)
remoteIO m = App $ fmap Here m
\end{code}
